...
var oplib = (function() {
	...
	oplib.fn = oplib.prototype = {
		...
		// Entfernt die ausgewählten aus dem DOM
		remove : function() {
			return this.each(function() {
	...
	oplib.fn.Form.updateData = function(elem) {
		...
		for (var i = 0; i < nodeOrder.length; i++) {
			...
			var node = nodeOrder[i].node;
			if (nodeData.before) {
				oplib.fn.Form.before(nodeData.id, nodeData.before, elem, true);
			}
			if (nodeData.after) {
				oplib.fn.Form.after(nodeData.id, nodeData.after, elem, true);
			}
			if (nodeData.first) {
				oplib.fn.Form.first(nodeData.id, elem, true);
			}
			if (nodeData.last) {
		...
		// Children
			...
			for (var i = 0; i < nodeOrder.children.length; i++) {
				if (!nodeOrder.children[i].remove) {
					...
					OPLib(nodeOrder.children[i].parent).append(nodeOrder.children[i].node);
				}
			}
		}
		return false;
